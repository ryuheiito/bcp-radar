"""
BCP RADAR — ハザードリスク分析 API v4
"""
from fastapi import FastAPI, HTTPException, Query
from fastapi.middleware.cors import CORSMiddleware
import httpx, asyncio, logging, ssl
...
async def fetch_jshis_pshm(lat, lon):
    try:
        async with new_client() as c:
        ...
        return {"i45": pct("T30_I45_PS"), "i50": pct("T30_I50_PS"),
                "i55": pct("T30_I55_PS"), "i60": pct("T30_I60_PS"), "si3": fv("T30_P03_SI")}
    except Exception as e:
        logger.warning(f"pshm失敗: {type(e).__name__}: {e}")
        return None


async def fetch_jshis_sstrct(lat, lon):
    try:
        async with new_client() as c:
...
async def fetch_jshis_landslide(lat, lon):
    try:
        async with new_client() as c:
...
async def fetch_flood_depth(lat, lon):
    try:
        async with new_client() as c:
...
def score_earthquake(pshm, sstrct):
...
def score_landslide(landslide, elev):
    score = 10
...
async def analyze(lat: float = Query(...), lon: float = Query(...)):
    ...
    elev, pshm, sstrct, landslide, flood = await asyncio.gather(
    ...
    return {
        ...
        "sources": ["J-SHIS https://www.j-shis.bosai.go.jp/", "浸水ナビ https://suiboumap.gsi.go.jp/"],
    }
